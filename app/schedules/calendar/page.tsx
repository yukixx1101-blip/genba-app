...
export default function ScheduleCalendarPage() {
  ...
  const normalizeDbDate = (dateText: string) => {
    if (!dateText) return ''
  ...
  const selectedSchedules = useMemo(() => {
    return data.filter((item) => normalizeDbDate(item.date) === selectedDateText)
  }, [data, selectedDateText])

  const hasSchedule = (date: Date) => {
    const calendarDateText = formatDate(date)
    return data.some((item) => normalizeDbDate(item.date) === calendarDateText)
  }
