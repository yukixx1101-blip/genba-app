...
export default function MonthlyReportsPage() {
  ...
  const summaries = useMemo<WorkerSummary[]>(() => {
    ...
    return Object.values(grouped).sort((a, b) => a.workerName.localeCompare(b.workerName, 'ja'))
  }, [reports, workerMap])

  const totalReports = reports.length
  const totalPhotos = reports.filter((item) => item.photo_url).length
  const totalSites = useMemo(() => {
    return new Set(reports.map((item) => item.site).filter(Boolean)).size
  }, [reports])
        ...
        {loading ? (
          ...
          <>
            ...
            >
              ...
              >
                <div style={{ fontSize: 13, color: '#666666' }}>日報件数</div>
                <div style={{ fontSize: 24, fontWeight: 700, marginTop: 4 }}>{totalReports}</div>
              </div>
              ...
              >
                <div style={{ fontSize: 13, color: '#666666' }}>現場数</div>
                <div style={{ fontSize: 24, fontWeight: 700, marginTop: 4 }}>{totalSites}</div>
              </div>
              ...
              >
                <div style={{ fontSize: 13, color: '#666666' }}>写真枚数</div>
                <div style={{ fontSize: 24, fontWeight: 700, marginTop: 4 }}>{totalPhotos}</div>
              </div>
            </div>
